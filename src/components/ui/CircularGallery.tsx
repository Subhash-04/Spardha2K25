...
class App {
  ...
  planeGeometry!: Plane;
  medias: Media[] = [];
  mediasImages: { image: string; text: string; onClick?: () => void }[] = [];
  ...
  createMedias(
    items: { image: string; text: string; onClick?: () => void }[] | undefined,
    bend: number = 1,
    textColor: string,
    borderRadius: number,
    font: string
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    const distance = (this.start - x) * (this.scrollSpeed * 0.1); // Increased sensitivity
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    // Detect click vs drag (short duration and small movement)
  ...
  handleClick(e: MouseEvent | TouchEvent) {
    ...
    const currentIndex = Math.round(Math.abs(this.scroll.current) / width) % this.medias.length;
    const centerMedia = this.medias[currentIndex];
    
    if (centerMedia && centerMedia.onClick) {
      centerMedia.onClick();
    }
  }
  ...
  update() {
    ...
    if (this.autoRotate && !this.isDown) {
      this.scroll.target += this.autoRotateSpeed * 0.05;
    }
